...
void string_copy(char* S, char* tcpy, int tcpy_size) {
	for (int i=0; i<tcpy_size; i++) {S[i] = tcpy[i];}
}
		...
		switch (*(ptr+1)) {
			case 's':
				...
				if (str_size >= 2) {
					...
					//string_realloc(S, S->size+1 + (str_size-2)); BORRAR
					string_grow(S, str_size-2);
					string_print_raw(S);
				...
				if (str_size >= 2) string_shift(ptr+2, str_size-2);
					...
					printf("BEFORE SHIFT\n"); string_print_raw(S); //BORRAR
					string_shift_left(ptr+2, 2-str_size);

					printf("AFTER SHIFT\n"); string_print_raw(S); //BORRAR
					//string_realloc(S, S->memsize-(2-str_size)); BORRAR
					string_shrink(S, 2-str_size);
					S->c_str[S->memsize-1] = 0;

					puts("left"); //BORRAR
					ptr = strchr(S->c_str, '%'); // after realloc we have to refresh the address.
				}
				string_print_raw(S);

				//fill
				string_copy(ptr, str, str_size); 
				S->size += str_size-2;
				string_print_raw(S);puts(""); //BORRAR
				}
				break;
			case 'd':
				break;
